...
describe("FlightStore", function() {
   describe("handles LOD changes from moving the globe", function() {
      it("by creating the camera change listener", function() {
         expect(viewer.camera.changed.numberOfListeners).toBeGreaterThanOrEqual(
            2
         );
      });
      ...
      xit("by responding to camera changes", function(done) {
         ...
         expect(
            flightStore.detailedFlights.has(FlightAPosition1.body.geohash)
         ).toBeFalsy();
   ...
   describe("routes update messages", () => {
      it("by routing position updates", () => {
         ...
         const flightC = flightStore.flightData.get(
            FlightCPosition1.icao
         ) as FlightRecord;
   ...
   describe("handles flight records", function() {
      it("by creating flight records", function() {
         expect(flightStore.flightData.size).toEqual(2);
         expect(
            flightStore.flightData.get(FlightAPosition1.icao)
         ).toBeDefined();
         expect(
            flightStore.flightData.get(FlightBPosition1.icao)
         ).toBeDefined();
         expect(flightStore.flightData.get("nonexistant")).not.toBeDefined();
      });

      it("by storing flight positions", function() {
         const flightARecord = flightStore.flightData.get(
            FlightAPosition1.icao
         );
         const flightBRecord = flightStore.flightData.get(
            FlightBPosition1.icao
         );
         if (
            flightARecord &&
            flightARecord.positions &&
            flightBRecord &&
            flightBRecord.positions
         ) {
            expect(flightARecord.positions.length).toEqual(1);
            expect<FlightPosition>(FlightAPosition1.body).toEqual(
               flightARecord.positions[0]
            );
            expect(flightBRecord.positions.length).toEqual(1);
            expect<FlightPosition>(FlightBPosition1.body).toEqual(
               flightBRecord.positions[0]
            );
         } else {
      ...
      it("by updating flight positions", function() {
         ...
         if (flightARecord && flightARecord.positions) {
            ...
            expect<FlightPosition>(FlightAPosition2.body).toEqual(
               flightARecord.positions[1]
            );
         } else {
      ...
      it("by storing demographic data for a flight which received position data first", function() {
         ...
         expect(flightRecord.demographic).toEqual({
            destination: "",
            origin: "",
            model: ""
         });
      ...
      it("by storing demographic data for a flight before receiving position data", function() {
         expect(
            flightStore.flightData.get(FlightCDemographic.icao)
         ).not.toBeDefined();
   ...
   describe("handles selection criteria", function() {
      it("by updating the selected flight map", function() {
         expect(
            flightStore.selectedFlights.has(FlightAPosition1.icao)
         ).toBeFalsy();
      ...
      it("by updating the detailed flight map", function() {
         expect(
            flightStore.detailedFlights.get(FlightAPosition1.body.geohash)
         ).not.toBeDefined();
         const testMap = new Map<string, boolean>([
            [FlightAPosition1.body.geohash, true]
         ]);
         ...
         expect(
            flightStore.detailedFlights.get(FlightBPosition1.body.geohash)
         ).not.toBeDefined();
      });
   });
   ...
   describe("handles display setting updates", function() {
      ...
      describe("for labels", function() {
         it("should update the label display options", function() {
            expect(flightStore.labelDisplayOptions).toEqual(
               LabelDisplayOptionDefaults
            );
            const newOptions = {
               color: "#ff6699"
